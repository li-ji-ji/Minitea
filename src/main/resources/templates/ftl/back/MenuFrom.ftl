...
<form class="layui-form" style="margin-top:50px;" action="" lay-filter="example">
		...
		<#switch menuitem.DATA_TYPE>
			<#case "tinyint">  
		    	<div class="layui-form-item">
			    	<label style="width:100px" class="layui-form-label">${menuitem.column_comment}</label>
			    	<div class="layui-input-block">
			      		<input type="checkbox" name="${menuitem.column_name}" id="${menuitem.column_name}" lay-skin="switch" lay-text="启用|停用">
			    	</div>
			  	</div>
          	<#break>   
		<#default>  
			<div class="layui-form-item">
	    		<label style="width:100px" class="layui-form-label">${menuitem.column_comment}</label>
	    		<div class="layui-input-block">
	      			<input type="text" name="${menuitem.column_name}" style="width:50%" id="${menuitem.column_name}" placeholder="请输入${menuitem.column_comment}" autocomplete="on" class="layui-input">
	    		</div>
	  		</div>
		</#switch>
  ...
  <!-- <div class="layui-form-item layui-form-text">
    ...
    <div class="layui-input-block">
      <textarea placeholder="请输入状态" type="number" class="layui-textarea" name="status" id="status"></textarea>
    </div>
  </div> -->
...
layui.use(['form', 'layedit', 'laydate'], function(){
  ...
  form.on('submit(demo1)', function(data){
	  ...
	  if(status==0){
		  ...
		  $.ajax({
			  "url": "http://92.68.10.110:8080/menu/addMenuAll",
			  "data":"menu="+menuJSON,
			  "type":"post",
	    	  "dataType" : "text",
	    	  "async" :false,
