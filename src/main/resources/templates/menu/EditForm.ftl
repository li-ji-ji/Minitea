...
<div class="container">
	<form role="form" action="/menu/edit" method="post">
	  <div class="form-group">
	    ...
	    <input type="text" readonly class="form-control" name="id" id="id" value="${menu.id?if_exists}" placeholder="请输入菜单名称">
	  </div>
	  <div class="form-group">
	    <label for="menuName">菜单名称</label>
	    <input type="text" class="form-control" name="menuName" id="menuName" value="${menu.menuName?if_exists}" placeholder="请输入菜单名称">
	  </div>
	  <div class="form-group">
	    <label for="menuUrl">跳转地址</label>
	    <input type="text" class="form-control" name="menuUrl" id="menuUrl" value="${menu.menuUrl?if_exists}" placeholder="请输入跳转地址">
	  </div>
	  <div class="form-group">
	    <label for="menuSort">菜单序号</label>
	    <input type="number" class="form-control" name="menuSort" id="menuSort" value="${menu.menuSort?if_exists}" placeholder="请输入菜单序号">
	  </div>
	  <div class="form-group">
	    <label for="pId">父级菜单编号</label>
	    <input type="number" class="form-control" value="${menu.pId!}" name="pId" id="pId" placeholder="请输入父级菜单编号">
	  </div>
	  <div class="form-group">
	    <label for="menuStatus">菜单状态</label>
	    <input type="number" class="form-control" name="menuStatus" id="menuStatus" value="${menu.menuStatus?if_exists}" placeholder="请输入菜单状态">
	  </div>
	  <div class="form-group">
	    <label for="menuImg">菜单图片</label>
        <div class="icon-box"><input data-placement="bottomRight" name="menuImg" class="form-control icp icp-auto" style="width: 90%;" value="${(menu.menuImg!)?html}" type="text" /><span class="input-group-addon"><i class="${(menu.menuImg!)?html}"></i></span></div>
	  </div>
	  <div class="form-group">
	    <label for="menuIntro">菜单描述</label>
	    <textarea class="form-control" name="menuIntro" id="menuIntro" rows="3">${menu.menuIntro?if_exists}</textarea>
	  </div>
